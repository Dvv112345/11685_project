...
class DDPMScheduler(nn.Module):
    ...
    def __init__(
        ...
        if self.beta_schedule == 'linear':
            ...
            t = torch.linspace(0, num_train_timesteps, steps, dtype=torch.float32)
            f_t = torch.cos(((t / num_train_timesteps + s) / (1 + s)) * (torch.pi / 2)) ** 2
            alphas_cumprod = f_t / f_t[0]
            betas = 1 - (alphas_cumprod[1:] / alphas_cumprod[:-1])
            betas = betas.clamp(beta_start, beta_end)
    ...
    def set_timesteps(
        ...
        timesteps = np.linspace(self.num_train_timesteps - 1, 0, num_inference_steps, dtype=int)
    ...
    def _get_variance(self, t):
        ...
        if t == 0:
            alpha_prod_t_prev = torch.tenosr(1)
    ...
    def add_noise(
        ...
        sqrt_alpha_prod = sqrt_alpha_prod[timesteps]
        while len(sqrt_alpha_prod.shape) < len(original_samples.shape):
            sqrt_alpha_prod = sqrt_alpha_prod.unsqueeze(-1)
            
        # TODO: get sqrt one miucs alphas
        sqrt_one_minus_alpha_prod = torch.sqrt(1-alphas_cumprod) 
        sqrt_one_minus_alpha_prod = sqrt_one_minus_alpha_prod[timesteps] 
